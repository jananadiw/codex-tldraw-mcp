...
`codex-tldraw-mcp` is a local stdio MCP server. It reads source files from the configured `repoPath` and writes generated `.tldr` and `.svg` files to `repoPath/boards`.
